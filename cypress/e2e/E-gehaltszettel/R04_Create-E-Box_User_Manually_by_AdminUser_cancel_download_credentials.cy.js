describe('R04_Admn user - Create E-Box User - Manually', () => {
  ...
  //   cy.get('#login').type(user.email).should('have.value', user.email);
  ...
  //         cy.log(`Inbox for ${user.email} is empty. Skipping delete.`);
  //       } else {
  //         // Emails exist → check delete button in main page
  //         cy.get('#delall').then(($btn) => {
  //           if (!$btn.is(':disabled')) {
  //             cy.wrap($btn).click({ force: true });
  //             cy.log(`All emails deleted for ${user.email}`);
  //           } else {
  //             cy.log(`Delete button disabled for ${user.email}`);
  //           }
  //         });
  //       }
  //     });
