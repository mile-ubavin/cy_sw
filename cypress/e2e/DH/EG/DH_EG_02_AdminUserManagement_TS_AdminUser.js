...
describe('Admin User', () => {
  it.only('Master user delete Admin user', () => {
    ...
    cy.wait(2500);
    // Conditional deletion: skip if paginator label is '0 of 0' or '0 von 0', else delete admin
    cy.get('.mat-mdc-paginator-range-actions>.mat-mdc-paginator-range-label')
      .invoke('css', 'border', '1px solid blue')
      .invoke('text')
      .then((text) => {
        const trimmedText = text.trim();
        if (/0 of 0|0 von 0/.test(trimmedText)) {
          cy.log('Admin user not found, skipping deletion.');
          return;
        } else {
          cy.log('Admin user found, proceeding with deletion.');
          cy.get('.action-buttons>button>.mdc-button__label')
            .should('exist')
            .filter((index, el) => {
              const text = Cypress.$(el).text().trim();
              return text === 'GDPR-Delete' || text === 'DSGVO-Löschung';
            })
            .first()
            .should('be.visible')
            .click({ force: true });
  ...
  it.only('DH - Create New User ', () => {
    ...
    cy.get('#admin-create-new')
      ...
      .wait(1000)
      .contains(/Create New Admin User|Neuen Admin Benutzer Anlegen/i) // DE + EN
      .click();
    ...
    cy.wait(500);
    cy.get(`#create-admin-firstName`).type(user.firstName);
    //Enter Last Name
    ...
    cy.wait(500);
    cy.get(`#create-admin-lastName`).type(user.lastName);
    ...
    cy.wait(500);
    cy.get(`#create-admin-username`).type(user.username);
    ...
    cy.wait(500);
    cy.get(`#create-admin-email`).type('invalid_email_format@yopmail');
    cy.get('#create-admin-email').focus().blur();
    cy.wait(2000);
    ...
    cy.get('#email-error')
      ...
      .then((text) => {
        expect(text.trim()).to.match(
          /Email format is invalid|Email-Format ist ungültig/i,
        );
      });
    cy.wait(500);

    //Enter valid email
    cy.get(`#create-admin-email`).clear().type(user.email);
    cy.wait(2000);
    // Validate error message for valid email format
    ...
    const roles = [
      /E-Box Viewer|E-Box ansehen/i,
      // /User Creation|Nutzeranlage/i,
      //   /Data Submitter|Versand/i,
    ];

    // STEP 2 — Loop through roles and select each one (by visible label in dropdown)
    roles.forEach((role) => {
      // Ensure dropdown is open before each selection
      // cy.get(`#create-admin-rights`).click({ force: true });
      cy.wait(500);
    ...
    cy.wait(2500);
    //Focus out of Roles dropdown to trigger any onBlur validations
    ...
    cy.intercept('POST', '**/person/fromGroup/xUser/AQUA').as(
      'createAdminUser',
    );
    //Check Success Message for Admin User creation
    cy.wait(1000);
    ...
    //******************    Create Admnin user with same username   ************************ */
    ...
    //Create Admnin user with same username to validate error message for duplicate username
    cy.get(`#create-admin-firstName`).clear().type(user.firstName);
    cy.get(`#create-admin-lastName`).clear().type(user.lastName);
    cy.get(`#create-admin-username`).clear().type(user.username);
    cy.get(`#create-admin-email`).clear().type(user.email);
    cy.wait(2000);
    ...
    // Validate error validation for duplicate username
    cy.get('#username-error')
      .should('be.visible')
      .invoke('text')
      .then((text) => {
    ...
    cy.get('div[color="error"]>div')
      ...
      .then((text) => {
        ...
        expect(trimmedText).to.match(
          /User could not be created. A user with this username already exists|Benutzername existiert bereits/i,
        );
      });
    cy.wait(2500);
    ...
    // cy.wait(1500);
    // cy.pause(); // Pause here to allow manual verification of logout before ending the test
  });

  it.only('Get Credentials from emails and Login as a New Admin', () => {
    const adminUser = Cypress.env('createAdminUser')[0];
    ...
    cy.wait(2500);
    cy.intercept('GET', 'https://**').as('visitURL');
    // cy.get('button[type="submit"]').click();
    ...
    cy.pause(); // Pause here to allow manual verification of the login page before proceeding with the test
    ...
    cy.get('@capturedUsername').then((username) => {
      cy.get('@capturedPassword').then((password) => {
        ...
        cy.wait(1500);
        cy.pause();
        cy.get('#login-button').click(); //end captured password
        // Wait for login to complete

        cy.pause(); // Pause here to allow manual verification of the login process before proceeding with the test
        ...
        //       },
        //       { en: 'Create/Update', de: 'Anlegen/Updaten' },
  ...
  //Check email content and delete email from Yopmail inbox
