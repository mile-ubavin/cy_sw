...
describe('DocuHub Registration', () => {
  ...
  describe('Happy path', () => {
    ...
    it('confirms account via Yopmail email', () => {
      cy.on('uncaught:exception', () => false);

      expect(registeredEmail, 'registered email must be set from before()').to.match(/@/);
    ...
    it('first time login to DocuHub', () => {
      ...
      cy.get('body').then(($body) => {
        ...
        cy.log(`Upload cards found in DOM: ${found.length > 0 ? found.join(', ') : 'NONE'}`);
        expect(found.length, 'at least one upload card must be present').to.be.greaterThan(0);
        found.forEach((id) => {
          cy.get(id).realHover();
          cy.contains(
            /Actions are not allowed|Aktionen sind nicht zulässig/i,
            { timeout: 5000 },
          ).should('be.visible');
        });
      });
      cy.log('✓ Hover hints verified — upload actions blocked until company prefix is set');
    });
  });
});
