...
// =============================================================================
// DH_EG Payment — Personal Document Upload with 305 Dictionary — NEW CARD (MASTERCARD)
// Scope: Upload PDF → auto-select 305 Dictionary → Confirm & send → Payment via NEW
// Mastercard → Orders List
// Difference vs stored card: expand "Weitere Zahlungsarten", pick new Kreditkarte,
// fill card form (Mastercard CY / 5555 4444 3333 1111 / 08 2026 / 121), then Bezahlen.
// =============================================================================

describe('DH_EG Payment — Upload 305 Dictionary (New Card — Mastercard)', () => {
  it('Upload PDF → select 305 dictionary → confirm & send → payment (new Mastercard) → orders list', () => {
    ...
    // ===== STEP 3: Upload PDF and select 305 dictionary =====
    ...
    cy.wait(1000);
    cy.get('li[data-value]', { timeout: 10000 }).should('have.length.at.least', 1);
    cy.get('li[data-value="PDFTABDictionary-305"]', { timeout: 15000 })
      .should('be.visible')
      .click({ force: true });
    ...
    cy.get('.e1sv65i456 table>tbody>tr')
      ...
      .then(($tds) => {
        ...
        cy.log(`Confirm & send row cells: ${JSON.stringify(cells)}`);
    ...
    cy.contains(/305_Dictionary/i).should('exist');
    ...
    // Use first tr (main document row) to avoid picking checkbox from the expanded details row
    cy.get('.e1sv65i456 table>tbody>tr')
      .first()
    ...
    // ===== STEP 8: Prices view → Next =====
    cy.contains(/Total\s*price|Gesamtpreis/i, { timeout: 10000 }).should('exist');
    ...
    // ===== STEP 9-12: Payment gateway — NEW CARD flow (Mastercard) =====
    ...
    // Card data: Mastercard CY / 5555 4444 3333 1111 / 08 2026 / 121.
    ...
    cy.iframe(PAYMENT_IFRAME)
      ...
      .should('be.visible')
      .clear({ force: true })
      .type('Mastercard CY', { force: true });
    cy.log('[STEP 11] Filled Karteninhaber: Mastercard CY');
    ...
    cy.iframe(PAYMENT_IFRAME)
      ...
      .should('not.be.empty')
      .then((body) =>
        cy.wrap(body).find('input').type('5555444433331111', { force: true }),
      );
    cy.log('[STEP 11] Filled Kreditkartennummer: 5555 4444 3333 1111');
    ...
    const ORDER_TIMESTAMP_RE = /(\d{2})\.(\d{2})\.(\d{4})[,\s]+(\d{2}):(\d{2}):(\d{2})/;
    ...
    cy.then(() => {
      ...
      cy.log(`[STEP 16] Polling for FRESH order (date=${todayStr}, min-time=${new Date(cutoffMs).toLocaleTimeString()}, max=${maxMs}ms)`);
    ...
    cy.then(() => {
      ...
      cy.get('table tbody tr').then(($rows) => {
        ...
        cy.log(`[STEP 17] Fresh rows (date=${todayStr}, ts>=${new Date(cutoffMs).toLocaleTimeString()}): ${freshRows.length}`);
        expect(freshRows.length, `At least 1 fresh order row (${todayStr} after ${new Date(cutoffMs).toLocaleTimeString()})`).to.be.greaterThan(0);
        ...
        if (rowTs !== null) {
          ...
          cy.log(`Order time: ${new Date(rowTs).toLocaleTimeString()} | Payment completed: ${paymentCompletedAt.toLocaleTimeString()} | Diff: ${diffMin.toFixed(2)} min`);
          ...
          expect(diffMin, 'Order timestamp should be within 10 min of payment completion').to.be.within(-10, 5);
        } else {
        ...
        cy.get('@confirmPrice').then((confirmPrice) => {
          ...
          cy.log(`Orders List price: "${priceText.trim()}" → ${actual} | Expected: ${confirmPrice} → ${expected}`);
    ...
    cy.then(() => {
      ...
      cy.get('table tbody tr').then(($rows) => {
        const freshRows = getFreshRows($rows, todayStr, cutoffMs);
        cy.log(`[STEP 18] Fresh rows to mark (date=${todayStr}, ts>=${new Date(cutoffMs).toLocaleTimeString()}): ${freshRows.length}`);
    ...
    // STEP 19: Stay on page 5 seconds
    cy.wait(5000);
