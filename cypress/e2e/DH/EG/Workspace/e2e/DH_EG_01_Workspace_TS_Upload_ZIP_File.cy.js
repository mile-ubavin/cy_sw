...
/**
 * DH_EG_01 Workspace — Upload ZIP file (ServiceLine + XML inside)
 *
 * Refactored from the original R16_Upload_ZIP_file.js (SW Admin upload) per
 ...
 *
 * Coverage:
 *   TC01 — DH Admin uploads ZIP, gets warning for invalid invoices, sends
 *   TC02 — E-Box user opens the resulting delivery
 *   TC03 — Yopmail validation of Versandreport e-Gehaltszettel email
 */

describe('DH - Admin uploads ZIP file (ServiceLine + XML)', () => {
  // TC01 — DH Admin upload ZIP, expect warning, send delivery
  it('DH - Upload ZIP file as Admin', () => {
    ...
    // Upload ZIP via DH-specific custom command (uses input[type="file"] selector)
    cy.DHuploadZipFile();
    ...
    // Verify warning message for invalid invoices in the ZIP
    ...
    cy.get('li[role="menuitem"]')
      ...
      .click();
    cy.url().should('include', Cypress.env('dh_baseUrl'));
    cy.log('DH ZIP upload finished successfully.');
  });
  ...
  it('E-Box user opens the uploaded delivery', () => {
    ...
    cy.wait(['@getIdentifications2'], { timeout: 57000 }).then(
      (interception) => {
        expect(interception.response.statusCode).to.eq(200);
      },
    );
