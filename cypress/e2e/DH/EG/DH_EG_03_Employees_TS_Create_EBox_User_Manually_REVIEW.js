...
// =============================================================================
// REVIEW VERSION — Optimizovana kopija originala
// Originalni fajl: DH_EG_03_Employees_TS_Create_EBox_User_Manually.js
//
// Izmene u odnosu na original:
//  1. CRITICAL FIX: `.cdk-row` (dodata tačka) — original koristio `cdk-row` bez tačke
//  2. CRITICAL FIX: cy.intercept dodat u searchAndDeleteUser — original IT (Delete)
//     koristio samo cy.wait(2000); DOM se proveravao pre API responsa → false negative
//     ("user postoji ali se ne prikazuje u rezultatima")
//  3. IT REDOSLED: "Delete Already created Users" premešten na PRVU poziciju
//     (staro IT1 + staro IT6 spojeni u jedan SW-Master login — nema duple sesije)
//  4. usersToDelete uključuje oba test korisnika iz env (createUser + createUserNoAddress)
//  5. dismissReleaseNotePopup i dismissCookieBar kao reusable helpers
//  6. Debug styling uklonjen (invoke style 'border: 2px solid black')
//  7. cy.url() bez asercije uklonjen
//  8. Company prefix selekcija: env-based match umesto hardkodiranog 'aqua - aqua'
//  9. Zakomentarisan kod (~120 linija) uklonjen
// =============================================================================

describe('Create e-Box user manually [REVIEW]', () => {
  // ---------------------------------------------------------------------------
  // Shared helpers
  // ---------------------------------------------------------------------------

  /**
   * Traži korisnika po userName i briše ga ako postoji.
   *
   * FIX: cy.intercept se registruje PRE submit-a → čekamo stvarni API response
   * pre nego što čitamo DOM (eliminisan false negative "user postoji, ne prikazuje se").
   */
  const searchAndDeleteUser = (userName) => {
    ...
    // DOM-render buffer after API response
    cy.wait(1500);
    ...
    cy.get('body').then(($body) => {
      if ($body.find('.cdk-row').length === 0) {
        ...
        // FIX: '.cdk-row' — original koristio 'cdk-row' bez tačke
  ...
  const dismissReleaseNotePopup = () => {
    cy.get('body').then(($body) => {
      if ($body.find('.release-note-dialog__close-icon').length > 0) {
        ...
        cy.log('Release note popup is NOT present');
      }
    });
  };
  ...
  // ---------------------------------------------------------------------------
  // IT1 — Precondition: Delete any leftover test users before suite starts
  //        (merged former IT1 + former IT6 — single SW-Master login session)
  // ---------------------------------------------------------------------------
  it('Login As Master User - Delete Already created Users', () => {
    ...
    // Build usersToDelete from env — includes both test users
    const usersToDelete = [
      Cypress.env('createUser')?.[0]?.username,
      Cypress.env('createUserNoAddress')?.[0]?.username,
    ].filter(Boolean);
  ...
  // ---------------------------------------------------------------------------
  // IT2 — DH: Create New E-box User manually
  // ---------------------------------------------------------------------------
  it('DH - Create New E-box User manually', () => {
    ...
    // Company prefix — env-based match, no hardcoded strings
    ...
    cy.get('div[role="dialog"]>div:nth-of-type(2)>div>div>div')
      ...
      .invoke('text')
      .then((text) => {
        expect(text.trim()).be.match(/Download user data.|Benutzersdaten herunterladen./);
      });
    ...
    cy.task('getDownloadedPdf', {
      ...
      cy.readFile(filePath, 'binary').then((pdfBinary) => {
        const pdfBlob = Cypress.Blob.binaryStringToBlob(pdfBinary, 'application/pdf');
  ...
  // ---------------------------------------------------------------------------
  // IT3 — Yopmail: Confirm email and Change password
  // ---------------------------------------------------------------------------
    ...
    cy.iframe('#ifmail')
      ...
      .then((innerText) => {
        ...
        cy.iframe('#ifmail')
          ...
          .invoke('attr', 'href')
          .then((href) => {
            cy.log(`Confirmation link href: ${href}`);
          });
        ...
        // External page load — hard wait unavoidable for external dependency
        cy.wait(15000);
        ...
        cy.iframe('#ifmail')
          ...
          .invoke('attr', 'href')
          .then((href) => {
            cy.log(`Password reset link href: ${href}`);
          });
  ...
  // ---------------------------------------------------------------------------
  // IT5 — DH: Try to Create New User when accountNumber already exists
  // ---------------------------------------------------------------------------
