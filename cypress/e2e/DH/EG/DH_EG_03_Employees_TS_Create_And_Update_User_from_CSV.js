describe('Admin User - Create User via CSV', () => {
  ...
  it('Login As AdminUser to DH - Create Users from CSV file', () => {
    ...
    // Register intercept BEFORE uploading the file so the request cannot be missed
    ...
    // Wait for request & assert — guard against undefined response on server error
    cy.wait('@uploadCSV', { timeout: 15000 }).then((interception) => {
      if (interception && interception.response) {
        expect(interception.response.statusCode).to.eq(200);
        cy.log(
          `Successful import – status: ${interception.response.statusCode}`,
        );
      } else {
        cy.log(
          'Warning: uploadCSV response was undefined – request may have failed or was aborted',
        );
        throw new Error('uploadCSV request did not receive a valid response');
      }
    });
    ...
    //select Company prefix - wait for combobox to render after CSV upload
    cy.findByRole(
      'combobox',
      { name: /Company Prefix|Firmenpräfix/i },
      { timeout: 20000 },
    )
      .should('be.visible')
      .click({ force: true });
    ...
    cy.findByRole('combobox', { name: /Company Prefix|Firmenpräfix/i })
      .should('be.visible')
      .click({ force: true });
    cy.wait(1000);
    cy.get("ul[role='listbox'] > li")
    ...
    cy.get('table tbody tr') // Select all table body rows
      ...
      .within(() => {
        ...
        cy.get('td') // Get all table cells in the row
          ...
          .invoke('text') // Extract text content from the cell
          .then((text) => {
            // Process the extracted text — normalize to lowercase to ignore prefix casing (e.g. "Aqua" vs "aqua")
            expect(text.trim().toLowerCase()).to.equal('aquaottotestuser'); // Assert username = company prefix + accountNumber
            cy.log(
              `✓ Username verified: ${text.trim()} (normalized: aquaottotestuser)`,
  ...
  it('Login As AdminUser to DH - Edit/Update User from CSV file', () => {
    ...
    // Wait for combobox to render after CSV upload, then click
    cy.findByRole(
      'combobox',
      { name: /Company Prefix|Firmenpräfix/i },
      { timeout: 15000 },
