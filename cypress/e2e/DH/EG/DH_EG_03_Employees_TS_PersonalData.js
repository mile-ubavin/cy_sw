...
describe('Login to DH using keycloak and upload-send PDF dictionary', () => {
  ...
  it('DH - Crete User from CSV file', () => {
    ...
    //Wait for BERT user logs to confirm that the dashboard has loaded
    cy.wait('@bertUserLogs', { timeout: 15000 }).then((interception) => {
      expect(interception.response.statusCode).to.eq(200);
      cy.log('Login successful, BERT user logs loaded');
    });

    cy.url().should('include', `${Cypress.env('dh_baseUrl')}home`);
    cy.wait(1000);

    //Switch to employees table
    cy.intercept('POST', '**/group').as('loadEmployees');
    cy.get('#nav-employees')
      .should('be.visible')
      .invoke('attr', 'style', 'border: 2px solid black; padding: 2px;') // highlight element
      .wait(1000)
    ...
    cy.wait('@loadEmployees', { timeout: 15000 }).then((interception) => {
      expect(interception.response.statusCode).to.eq(200);
      cy.log('Employees table loaded');
    });
    ...
    cy.get('div[role="dialog"]>div:nth-child(2)>div>div>div>span')
      ...
      .then((text) => {
        ...
        // Validate that the text matches either the German or English version
        expect(trimmedText).to.match(
          /1 user was updated|Versucht 1 nicht existierenden Benutzer zu aktualisieren/i,
