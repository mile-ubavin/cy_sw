// =============================================================================
// DH_EG_04 – DocuHub Self-Registration Test Suite
// =============================================================================
//
// PREREQUISITES – add the following snippets to their respective files:
//
// ── 1. cypress.config.js  (inside setupNodeEvents) ───────────────────────────
//
//   on('task', {
//     async solveFriendlyCaptcha(sitekey) {
//       const puzzleRes = await fetch(
//         `https://api.friendlycaptcha.com/api/v1/puzzle?sitekey=${sitekey}`,
//       );
//       const { data } = await puzzleRes.json();
//       const puzzleBase64 = data.puzzle;
//       const puzzleBuffer = Buffer.from(puzzleBase64.split('.')[1], 'base64');
//       const crypto = require('crypto');
//       const difficulty = puzzleBuffer[0]; // first byte = difficulty
//       const puzzle = puzzleBuffer.slice(1);
//       let nonce = 0;
//       let solution = null;
//       while (true) {
...
//         const hash = crypto
//           .createHash('sha256')
//           .update(Buffer.concat([puzzle, nonceBuffer]))
//           .digest();
//         let zeroBits = 0;
...
//           solution = `${puzzleBase64.split('.')[0]}.${nonceBuffer.toString('base64')}`;
//           break;
//         }
//         nonce++;
//       }
...
//     cy.task('solveFriendlyCaptcha', 'FCMURNNP42EE0QOF', { timeout: 60000 })
//       .then((solution) => {
//         cy.get('input[name="frc-captcha-solution"]')
//           .invoke('val', solution)
//           .trigger('input')
//           .trigger('change');
//         // Patch the widget DOM state so React sees it as complete
