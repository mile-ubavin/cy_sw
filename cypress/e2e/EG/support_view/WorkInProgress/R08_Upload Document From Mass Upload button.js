describe('R05_Upload Document From Mass Upload button', () => {
  ...
  it('Admin User can Upload Document From Mass Upload button', () => {
    cy.fixture('supportView.json').as('payslipSW');
  ...
  it('HR Admin can Upload Document From Mass Upload button', () => {
    ...
    cy.get('.cdk-column-description>.cell-content-wrap>div>div')
      .invoke('text')
      .then((text) => {
        const companyDescription = text.trim(); // Ensure trimming
        cy.log(`Extracted Company Description: ${companyDescription}`);

        const toCompanies = [`${companyDescription} - AQUA`]; // Construct expected value
        cy.log(`Target Companies: ${toCompanies}`);
        ...
        // Capture and log upload datetime
        const now = new Date();
        ...
        // Set document title
        const title = `Document From MassUpload (pdf) - ${uploadDateTime}`;
        cy.log(`Title for the document: ${title}`);
        ...
        // **Use stored company description**

        // Step 3: Open dropdown
        cy.get('.broadcast-companies').click();
        cy.wait(1000);

        // Step 4: Deselect all checkboxes
        cy.get('mat-option[role="option"]>.mdc-list-item__primary-text')
          .should('exist')
          .each(($label) => {
            cy.wrap($label)
              ...
              .then(($checkboxInput) => {
                if ($checkboxInput.is(':checked')) {
                  cy.wrap($checkboxInput).click({ force: true });
                  cy.log(`Unchecked: ${$label.text().trim()}`);
                }
              });
          });
        ...
        // Step 5: Close dropdown to refresh UI state
        cy.get('body').click(0, 0);
        cy.wait(1000);

        // Step 6: Reopen dropdown
        cy.get('.broadcast-companies').click();
        cy.wait(1000);

        // Step 7: Select the target company
        cy.get('mat-option[role="option"]>.mdc-list-item__primary-text')
          .should('exist')
          .each(($label) => {
            const text = $label.text().trim();
            cy.log(`Dropdown Item: ${text}`);
